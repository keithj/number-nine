...
(defmacro n9-assocdr (key alist &rest args)
 "Return the cdr of the cons cell returned by calling (assoc KEY
ALIST ARGS)."
...
(defmacro with-n9-product (product &rest body)
 "Execute BODY with *current-n9-product* bound to PRODUCT."
 `(let ((*current-n9-product* ,product))
    ,@body))

(defun* make-n9-product (name owner &key backlog definition-done)
 "Return a new product."
 (let ((dod (or definition-done
                (list "Unit tests written and green."
                      "Public interface documented."
                      "Committed into a VCS."
                      "All tests green on the CI server."
                      "All acceptance criteria met."))))
   (cl-pairlis '(name owner definition-done stories backlog
                      sprints story-counter sprint-counter)
               (list name owner dod backlog backlog
                     () 0 0))))
...
(defun n9-product-definition-of-done (product)
 "Return the PRODUCT definition of done."
 (n9-assocdr 'definition-done product))

(defun n9-story-owners (product)
 "Return a list of all story owners for PRODUCT."
...
(defun find-n9-sprint (sprint-number product)
 "Return sprint SPRINT-NUMBER of PRODUCT."
...
(defun find-n9-story (story-number product)
 "Return story STORY-NUMBER of PRODUCT."
...
(defun* make-n9-sprint (product &key duration number start)
 "Return a new sprint for PRODUCT."
...
(defun n9-story-number (story)
 "Return the unique number for STORY."
 (n9-assocdr 'number story))

(defun n9-story-product (story)
 "Return the product of the STORY."
 (n9-assocdr 'product story))

(defun n9-story-owner (story)
 "Accessor for the STORY owner. This is normally the same as the
product owner."
...
(defun n9-story-role (story)
 "Return the role clause describing the STORY. This indicates the
STORY owner's role e.g. user, developer, development team."
 (n9-assocdr 'role story))

(defun n9-story-goal (story)
 "Return the goal clause describing the STORY."
 (n9-assocdr 'goal story))

(defun n9-story-benefit (story)
 "Return the benefit clause describing the STORY."
 (n9-assocdr 'benefit story))

(defun n9-story-criteria (story)
 "Accessor for the list of acceptance criteria for completion of the
STORY."
...
(defun n9-story-priority (story)
 "Return the current priority of the STORY set by the owner."
...
(defun n9-story-created (story)
 "Return the date the STORY was created."
 (n9-assocdr 'created story))

(defun n9-story-description (story)
  "Return a sentence describing the STORY, constructed from its role, goal
and benefit."
...
(defun describe-n9-story (role goal benefit)
  "Return a sentence constructed from a story its ROLE, GOAL and BENEFIT."
  (format "As a/an %s I/we want %s so that %s" role goal benefit))

(defun filter-n9-story (story &rest keys)
  "Return an alist containing only the conses of STORY matching KEYS."
  (cl-remove-if-not (lambda (key)
                      (member key keys)) story :key #'car))

(defun n9-iso8601-utc (epoch-time)
  "Return an ISO 8601 UTC string representation of EPOCH-TIME."
  (format-time-string "%Y-%m-%dT%TZ" epoch-time t))

;;; Interactive functions
(defun load-n9-product (filename)
 "Load a saved No. 9 product expression from file FILENAME and set
the current product to its value."
...
(defun save-n9-product (filename)
 "Save the current No. 9 product to file FILENAME."
 (interactive "FProduct file name: ")
...
(defun show-n9-product ()
  "Display the current product, including its Definition of Done."
  (interactive)
  (if (null (current-n9-product))
      (message "No product is loaded.")
    (print (mmd-print-product (current-n9-product)))))

(defun export-n9-stories (dirname)
 "Export all of the stories in the current product as MultiMarkdown files,
one per story, into directory DIRNAME. Existing files of the same
names will be overwritten. Files are named
...
(defun new-n9-product (name owner)
 "Create a new No. 9 product and sets it as the current product. The
NAME argument should be short, descriptive name and OWNER should be a
string identifying the product owner, such as an email address."
 (interactive "MProduct name: \nMProduct owner: ")
 (setq *current-n9-product* (make-n9-product name owner)))

(defun show-n9-story (story-number)
 "Display story STORY-NUMBER from the current product as
MultiMarkdown."
...
(defun add-n9-story (role goal benefit)
 "Add a new story to the current product, prompting 'As a/an <ROLE>
I/we want to <GOAL> so that <BENEFIT>. The story is added to the
product story list, not to the backlog or sprint lists."
...
(defun add-n9-story-criterion (story-number criterion)
 "Add a new acceptance CRITERION to story STORY-NUMBER, which must exist in
the current product."
 (interactive "nStory number: \nMCriterion: ")
...
(defun prioritise-n9-story (story-number priority)
 "Modify the priority number of story STORY-NUMBER in the current
product to PRIORITY."
 (interactive "nStory number: \nnNew priority: ")
 (let ((product (current-n9-product)))
...
(defun transfer-n9-story (story-number owner)
 "Modify ownership of story STORY-NUMBER in the current product to
OWNER."
 (interactive "nStory number: \nMNew owner: ")
 (let ((product (current-n9-product)))
...
(defmacro define-mmd-header (rank)
 (let ((fname (intern (format "mmd-header-%d" rank)))
       (docstring
        (format "Return a level %d header as a string, with newline" rank)))
   `(defun ,fname (str)
      ,docstring
...
(defun mmd-print-product (product)
  (with-output-to-string
    (princ (mmd-header-1 (n9-product-name product)))
    (princ "\n")
    (princ (format "Owner: %s\n" (n9-product-owner product)))
    (princ (format "Status: %d sprints to date, %d stories, %d in current backlog\n"
                   (list-length (n9-product-sprints product))    
                   (list-length (n9-product-stories product))
                   (list-length (n9-product-backlog product))))
    (princ "\n")
    (princ (mmd-header-2 "Definition of Done"))
    (princ "\n")
    (princ (mmd-enum-list (n9-product-definition-of-done product)))))
